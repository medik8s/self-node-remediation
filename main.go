...
func init() {
	...
	utilruntime.Must(selfnodev1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.BoolVar(&isManager, "is-manager", false,
		"Used to differentiate between the self node agents that runs in a daemonset to the 'manager' that only"+
	...
	if isManager {
		initSelfNodeManager(mgr)
	} else {
		initSelfNodeAgent(mgr)
	}
...
func initSelfNodeManager(mgr manager.Manager) {
	...
	if err := (&controllers.SelfNodeConfigReconciler{
		Client:            mgr.GetClient(),
		Log:               ctrl.Log.WithName("controllers").WithName("SelfNodeConfig"),
		Scheme:            mgr.GetScheme(),
		InstallFileFolder: "./install",
		DefaultPpcCreator: newConfigIfNotExist,
		Namespace:         ns,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "SelfNodeConfig")
		os.Exit(1)
	}

	if err := newConfigIfNotExist(mgr.GetClient()); err != nil {
		setupLog.Error(err, "failed to create a default self node config CR")
		os.Exit(1)
	}
...
func initSelfNodeAgent(mgr manager.Manager) {
	setupLog.Info("Starting as a self node agent that should run as part of the daemonset")
...
// newConfigIfNotExist creates a new SelfNodeConfig object
...
func newConfigIfNotExist(c client.Client) error {
	...
	config := selfnodev1alpha1.NewDefaultSelfNodeConfig()
	config.SetNamespace(ns)

	err = c.Create(context.Background(), &config, &client.CreateOptions{})
	if err != nil && !apierrors.IsAlreadyExists(err) {
		return errors.Wrap(err, "failed to create a default self node config CR")
	}
	return nil
}
...
func newDefaultTemplateIfNotExist(c client.Client) error {
	...
	pprt := selfnodev1alpha1.NewDefaultRemediationTemplate()
	pprt.SetNamespace(ns)

	err = c.Create(context.Background(), &pprt, &client.CreateOptions{})
	if err != nil && !apierrors.IsAlreadyExists(err) {
		return errors.Wrap(err, "failed to create a default self node template CR")
	}
	return nil
}
