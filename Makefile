...
BUNDLE_IMG ?= quay.io/medik8s/self-node-operator-bundle:$(VERSION)

# Image URL to use all building/pushing image targets
IMG ?= quay.io/medik8s/self-node-operator:$(VERSION)
...
bundle: manifests operator-sdk kustomize
	...
	cd config/manager && $(KUSTOMIZE) edit set image controller=$(IMG)
	sed -r -i "s|createdAt: \".*\"|createdAt: \"`date "+%Y-%m-%d %T" `\"|;" ./config/manifests/bases/self-node.clusterserviceversion.yaml
