...
type SelfNodeRemediationConfigSpec struct {
	...
	// Note that remediation time is affected by other fields as well such as ApiCheckInterval, ApiServerTimeout ,MaxApiErrorThreshold, PeerDialTimeout and PeerRequestTimeout. Modifying SafeTimeToAssumeNodeRebootedSeconds without considering those fields will not necessarily affect the remediation time.
