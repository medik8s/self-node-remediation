...
func (r *SelfNodeRemediationReconciler) prepareReboot(ctx context.Context, node *v1.Node, snr *v1alpha1.SelfNodeRemediation) (ctrl.Result, error) {
	...
	if snr.Status.TimeAssumedRebooted.IsZero() {
		if err := r.updateTimeAssumedRebooted(ctx, node, snr); err != nil {
			return ctrl.Result{}, err
		}
	}
...
func (r *SelfNodeRemediationReconciler) updateTimeAssumedRebooted(ctx context.Context, node *v1.Node, snr *v1alpha1.SelfNodeRemediation) error {
	...
	toAssumeNodeRebooted, err := r.SafeTimeCalculator.GetTimeToAssumeNodeRebooted(ctx)
	if err != nil {
		return err
	}
